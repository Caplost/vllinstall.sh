...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help)
      ...
      echo "  --app-name <名称>      应用名称 (默认: deepseek_r1_int4)"
      echo "  --no-shutdown          不关闭已有部署"
      echo "  --debug                启用调试模式"
      echo "  --num-gpus <数量>      使用的GPU数量 (默认: 1)"
      echo "  --cpu-only             仅使用CPU (不推荐生产环境)"
...
if [ "$CPU_ONLY" = true ]; then
  echo -e "${YELLOW}注意: 使用CPU模式 (性能较低)${NC}"
fi
if [ "$DEBUG" = true ]; then
...
# 执行Python脚本
echo -e "${BLUE}执行命令: python $(dirname "$0")/ray-deepseek-r1-int4.py --model-path ${MODEL_PATH} --app-name ${APP_NAME} --port ${PORT} --num-gpus ${NUM_GPUS} ${PYTHON_ARGS}${NC}"
python $(dirname "$0")/ray-deepseek-r1-int4.py --model-path "${MODEL_PATH}" --app-name "${APP_NAME}" --port "${PORT}" --num-gpus "${NUM_GPUS}" ${PYTHON_ARGS}
