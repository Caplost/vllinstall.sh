...
logger = logging.getLogger("DeepSeek-R1-Int4")
...
# 解析命令行参数
parser = argparse.ArgumentParser(description="Deploy DeepSeek-R1-Int4-AWQ model with Ray Serve")
...
parser.add_argument("--cpu-only", action="store_true", help="Run on CPU only (not recommended for production)")
...
if not args.cpu_only and not torch.cuda.is_available():
    print_warning("检测不到CUDA GPU。将回退到CPU模式，这可能会导致性能极低。")
    print_warning("如果有GPU但检测不到，请检查CUDA环境配置。")
    args.cpu_only = True
...
try:
    ...
    @serve.deployment(
        ...
        ray_actor_options={"num_gpus": 0 if args.cpu_only else args.num_gpus},
    ...
    class DeepSeekInt4:
        def __init__(self):
            ...
            self.device = "cpu" if args.cpu_only else "cuda"
            ...
            try:
                ...
                self.model = LLM(
                    ...
                    tensor_parallel_size=1 if args.cpu_only else int(args.num_gpus),
