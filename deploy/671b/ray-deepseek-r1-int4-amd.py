...
def check_amd_gpu_availability():
    ...
    try:
        ...
        if not hasattr(torch, 'hip') or not torch.hip.is_available():
            ...
            if not hasattr(torch, 'version') or not hasattr(torch.version, 'hip'):
                print_error("当前PyTorch未编译HIP/ROCm支持，无法使用AMD GPU")
                return False
        ...
        try:
            ...
            if result.returncode != 0:
                print_warning("rocm-smi命令失败，尝试使用其他方法检测AMD GPU")
            else:
                print_info("检测到ROCm环境，rocm-smi命令可用")
        except FileNotFoundError:
            print_warning("未找到rocm-smi命令，但这不一定意味着无法使用AMD GPU")
            
        # 检查PyTorch是否能检测到AMD GPU设备
        device_count = 0
        if hasattr(torch, 'hip') and torch.hip.is_available():
            device_count = torch.hip.device_count()
        else:
            # 尝试直接使用CUDA API检查设备(在某些环境中，ROCm通过CUDA兼容层工作)
            try:
                device_count = torch.cuda.device_count()
            except:
                print_error("无法检测到GPU设备")
                return False
                
        if device_count == 0:
            print_error("未检测到可用的AMD GPU设备")
            return False
            
        print_success(f"检测到 {device_count} 个AMD GPU设备")
        # 尝试获取GPU名称(不是所有ROCm版本都支持)
        try:
            for i in range(device_count):
                if hasattr(torch.hip, 'get_device_name'):
                    gpu_name = torch.hip.get_device_name(i)
                else:
                    # 尝试使用CUDA兼容API
                    gpu_name = torch.cuda.get_device_name(i)
                print_info(f"  GPU {i}: {gpu_name}")
        except:
            print_warning("无法获取GPU详细信息，但设备已检测到")
            
        return True
    except Exception as e:
        print_error(f"检测AMD GPU时出错: {str(e)}")
        return False

# 解析命令行参数
parser = argparse.ArgumentParser(description="Deploy DeepSeek-R1-Int4 model with Ray Serve on AMD GPU")
...
parser.add_argument("--use-cuda-compat", action="store_true", default=True,
                    help="Use CUDA compatibility layer for ROCm (default: True)")
parser.add_argument("--rocm-device-map", type=str, default="auto",
                    help="Device mapping for ROCm (default: auto)")
parser.add_argument("--hip-visible-devices", type=str, 
                    help="Specify visible devices for HIP (comma-separated list)")
args = parser.parse_args()

# 设置AMD GPU的环境变量（如果提供了）
if args.hip_visible_devices:
    os.environ["HIP_VISIBLE_DEVICES"] = args.hip_visible_devices
    print_info(f"已设置 HIP_VISIBLE_DEVICES={args.hip_visible_devices}")

# 如果使用CUDA兼容层，设置相应环境变量
if args.use_cuda_compat:
    os.environ["HSA_OVERRIDE_GFX_VERSION"] = "10.3.0"
    print_info("已设置 HSA_OVERRIDE_GFX_VERSION=10.3.0 用于CUDA兼容层")
...
try:
    ...
    if hasattr(torch, 'version') and hasattr(torch.version, 'hip'):
        print_info(f"ROCm 版本: {torch.version.hip}")
    else:
        print_warning("PyTorch 可能未启用ROCm支持")
except Exception as e:
...
if not args.cpu_only:
    print_info("\n==== AMD GPU 检测 ====")
    if not check_amd_gpu_availability():
        print_error("\nAMD GPU检测失败。您有以下选择:")
        print_error("1. 确保已安装ROCm驱动和运行时")
        print_error("2. 确保PyTorch是ROCm版本或支持ROCm")
        print_error("3. 使用 --cpu-only 参数以CPU模式运行（不推荐生产环境使用）")
        print_error("4. 尝试使用 --use-cuda-compat 参数启用CUDA兼容层")
        sys.exit(1)
...
if total_detected_gpus == 0 and not args.cpu_only:
    print_warning("Ray集群未检测到任何GPU资源！这可能是因为:")
    print_warning("1. Ray无法正确识别AMD GPU (但仍然可以使用)")
    print_warning("2. 实际上没有可用的GPU资源")
    print_warning("\n尝试继续运行，但如果发生错误，请考虑使用 --cpu-only 参数")
...
if args.cpu_only:
    ...
    if use_distributed:
        ...
        gpus_per_replica = max(1, total_detected_gpus)  # 确保至少有1个GPU
        num_replicas = 1
        tensor_parallel_size = gpus_per_replica
    ...
    def __init__(self):
        ...
        try:
            ...
            if not args.cpu_only:
                ...
                if args.use_cuda_compat:
                    print_info("启用CUDA兼容层，适用于AMD GPU")
            ...
            if not args.cpu_only:
                llm_kwargs.update({
                    ...
                    "gpu_memory_utilization": 0.90,  # 调低AMD GPU的内存利用率以避免OOM
                    "dtype": args.dtype,  # 使用命令行参数指定的数据类型
                })
                
                # 为AMD GPU添加特定参数
                if args.rocm_device_map != "auto":
                    llm_kwargs.update({
                        "device_map": args.rocm_device_map,
                    })
            else:
                # CPU模式设置
                llm_kwargs.update({
