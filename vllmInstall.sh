...
check_pip_packages() {
    ...
    required_packages=("torch" "transformers" "vllm" "accelerate")
    missing_packages=()
...
show_installation_summary() {
    ...
    echo "5. 性能调优:"
    echo "   - 增加tensor-parallel-size参数等于GPU数量可提高多GPU性能"
    echo "   - 调整max-tokens可控制单次生成的最大长度"
    echo "   - 调整gpu-memory-utilization在0.0-1.0之间可控制显存使用率"
    echo ""
}
...
main() {
    clear
    echo "============================================================"
    echo "                Ubuntu vLLM 环境配置和检测工具                "
    echo "============================================================"
    echo ""
    echo "此脚本将帮助您检查和配置Ubuntu系统上的vLLM环境，包括:"
    echo "  - 检查系统环境"
    ...
    if [ "$MISSING_PACKAGES" -eq 1 ] || [ "$TORCH_CUDA_ERROR" -eq 1 ]; then
        print_warning "检测到必要依赖缺失，将自动安装vLLM及依赖..."
        install_vllm
        ...
        if [ "$saved_vllm_installed" = "1" ]; then
            ...
            read -p "是否重新安装vLLM? (y/n) [n]: " reinstall_choice
            ...
            print_info "建议安装vLLM及依赖以确保完整功能"
            read -p "是否安装vLLM及依赖? (y/n) [y]: " install_choice
    ...
    print_section "完成"
    print_success "Ubuntu vLLM环境配置和检测已完成!"
